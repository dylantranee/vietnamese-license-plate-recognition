...
This script processes video files to detect and recognize Vietnamese license plates
for both motorcycles and cars. It uses YOLOv8 for plate detection and EasyOCR for
text recognition, with extensive preprocessing and validation to handle various
plate formats and image quality issues.

Supported formats:
- Motorcycles: DD-LN-XXX.XX (e.g., 51-C5-123.45)
- Cars: DD-L-XXX.XX (e.g., 30-A-456.78)
- Old Cars: DD-L-NNNN (e.g., 30-M-4253)

Usage:
    python main.py
    
The script will process 'vehicle_video.mp4' and output 'output_with_licensev3.mp4'
with detected plates annotated on each frame.
"""
...
def deskew_plate(image):
    ...
    print("DEBUG: Applied perspective transform.")
    cv2.imwrite("debug_plate_deskewed.jpg", warped)
...
def preprocess_plate(plate_crop):
    ...
    if candidates:
        ...
        print(f"DEBUG: Median Height: {median_h:.1f}. Candidates: {len(candidates)}")
        ...
        if density_count > 0:
             ...
             print(f"DEBUG: Avg Density: {avg_density:.2f}")
    ...
    # Save debug output
    cv2.imwrite("debug_plate_final.jpg", plate_final)
...
def correct_plate_format(ocr_text):
    ...
    # Step 5: Contextual fix for 0↔9 confusion
    ...
    # Step 6: Remove phantom '1' artifact (common border detection error)
    ...
    # Step 7: Try different format interpretations based on length
...
def recognize_plate(plate_crop):
    ...
    Returns formatted plate string or empty string if recognition fails.
    """
...
# ============================================================================
# 6. PLATE STABILIZATION (TEMPORAL FILTERING)
# ============================================================================

# Buffer to store plate detections across frames
plate_history = defaultdict(lambda: deque(maxlen=10))
plate_final = {}

def get_box_id(x1, y1, x2, y2):
    """
    Generates a unique ID for a bounding box based on its location.
    Boxes within 10 pixels are considered the same plate.
    """
    return f"{int(x1/10)}_{int(y1/10)}_{int(x2/10)}_{int(y2/10)}"

def get_stable_plate(box_id, new_text):
    """
    Applies temporal filtering to reduce flickering.
    Returns the most common plate text from the last 10 frames for this box.
    """
    if new_text:
        plate_history[box_id].append(new_text)
        # Select most frequent result from history
    ...
    while cap.isOpened():
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                if plate_crop.size > 0 and stable_text:
                    # Draw text shadow for readability
                    cv2.putText(frame, stable_text, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 4)
                    # Draw text foreground
                    cv2.putText(frame, stable_text, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 2)
